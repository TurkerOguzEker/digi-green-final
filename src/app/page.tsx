          ...
          <div className="container">
              <div className="overview-grid">
                  <div className="overview-text">
                      <h2 style={{color:'#003399', marginBottom:'20px'}}>{content.home_overview_title || 'Sürdürülebilir Bir Gelecek İçin'}</h2>
                      <p className="lead" style={{color:'#555', marginBottom:'20px'}}>
                          {content.home_overview_text || 'DIGI-GREEN FUTURE, yerel yönetimlerin dijital dönüşümünü çevresel sürdürülebilirlik hedefleriyle birleştiren yenilikçi bir Erasmus+ projesidir.'}
                      </p>
                      <p style={{color:'#666', lineHeight:'1.6'}}>
                          {content.home_overview_text_2 || 'İklim değişikliği ile mücadelede yapay zeka ve mobil teknolojileri kullanarak, vatandaşların karar alma süreçlerine katılımını artırmayı hedefliyoruz.'}
                      </p>
                  </div>
          ...
          <div className="container">
              ...
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:'30px'}}>
                  ...
                  <div style={{background:'white', padding:'40px', borderRadius:'10px', boxShadow:'0 5px 15px rgba(0,0,0,0.05)', borderTop:'5px solid #27ae60'}}>
                      ...
                      <h3 style={{fontSize:'1.3rem', marginBottom:'15px'}}>{content.home_goal_1_title || 'Yeşil Dönüşüm'}</h3>
                      <p style={{color:'#666'}}>{content.home_goal_1_desc}</p>
                  </div>
                  {/* KUTUCUK 2 */}
                  <div style={{background:'white', padding:'40px', borderRadius:'10px', boxShadow:'0 5px 15px rgba(0,0,0,0.05)', borderTop:'5px solid #003399'}}>
                      ...
                      <h3 style={{fontSize:'1.3rem', marginBottom:'15px'}}>{content.home_goal_2_title || 'Dijital Okuryazarlık'}</h3>
                      <p style={{color:'#666'}}>{content.home_goal_2_desc}</p>
                  </div>
                  {/* KUTUCUK 3 */}
                  <div style={{background:'white', padding:'40px', borderRadius:'10px', boxShadow:'0 5px 15px rgba(0,0,0,0.05)', borderTop:'5px solid #f1c40f'}}>
                      ...
                      <h3 style={{fontSize:'1.3rem', marginBottom:'15px'}}>{content.home_goal_3_title || 'Katılımcı Yönetişim'}</h3>
                      <p style={{color:'#666'}}>{content.home_goal_3_desc}</p>
                  </div>
              </div>
          </div>
      </section>

      {/* --- ETKİLER VE MADDELER (DİNAMİK) --- */}
      <section className="section-padding bg-white">
          <div className="container">
              <div className="overview-grid">
                  <div style={{order:2}}>
                      ...
                      <ul className="project-details-list" style={{background:'white', padding:0, border:0}}>
                          <li><i className="fas fa-check-circle" style={{color:'#27ae60'}}></i> {content.home_impact_list_1 || 'Toplumsal iklim bilincinin artırılması.'}</li>
                          <li><i className="fas fa-check-circle" style={{color:'#27ae60'}}></i> {content.home_impact_list_2 || 'Mobil uygulamalar ile karbon ayak izinin takibi.'}</li>
                          <li><i className="fas fa-check-circle" style={{color:'#27ae60'}}></i> {content.home_impact_list_3 || 'Yerel yönetimlerde dijital araç kullanımının yaygınlaşması.'}</li>
                          {content.home_impact_list_4 && <li><i className="fas fa-check-circle" style={{color:'#27ae60'}}></i> {content.home_impact_list_4}</li>}
                      </ul>
                  </div>
                  <div style={{order:1}}>
                      <img src={content.home_impact_image || "https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&q=80"} alt="Etki" style={{borderRadius:'10px', boxShadow:'0 15px 30px rgba(0,0,0,0.1)'}} />
                  </div>
              </div>
          </div>
      </section>

      {/* --- CTA (ÇAĞRI - ARTIK DİNAMİK) --- */}
      <section style={{background:'linear-gradient(to right, #003399, #001a4d)', padding:'80px 0', color:'white', textAlign:'center'}}>
          <div className="container">
              <h2 style={{fontSize:'2.5rem', marginBottom:'20px', color:'white'}}>{content.home_cta_title || 'Yeşil Bir Gelecek İçin Bize Katılın'}</h2>
              <p style={{fontSize:'1.2rem', marginBottom:'30px', opacity:0.9, maxWidth:'700px', margin:'0 auto 30px'}}>
                  {content.home_cta_desc || "Kapaklı Belediyesi'nin öncülük ettiği bu dijital dönüşüm hareketinin bir parçası olun."}
              </p>
              <div style={{display:'flex', gap:'15px', justifyContent:'center'}}>
                  <Link href={content.home_cta_btn1_link || "/contact"} className="btn" style={{background:'#27ae60', color:'white', border:'none', fontSize:'1.1rem', padding:'15px 40px'}}>
                      {content.home_cta_btn1_text || "İletişime Geçin"}
                  </Link>
                  <Link href={content.home_cta_btn2_link || "/news"} className="btn btn-outline">
                      {content.home_cta_btn2_text || "Haberleri Takip Edin"}
                  </Link>
              </div>
          </div>
      </section>
