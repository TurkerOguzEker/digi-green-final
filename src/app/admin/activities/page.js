...
export default function AdminActivitiesPage() {
  ...
  async function saveItem(e) {
    e.preventDefault();
    const { id, ...data } = activityForm;
    let result = id ? await supabase.from('activities').update(data).eq('id', id) : await supabase.from('activities').insert([data]);
    ...
    setActivityForm({ id: null, title: '', title_en: '', type: 'Toplanti (TPM)', type_en: '', location: '', location_en: '', date: '', summary: '', summary_en: '', description: '', description_en: '', image_url: '' });
  }
  ...
  const NAV = [
    ...
    { id: 'news', label: 'Haberler', icon: 'fas fa-newspaper', group: 'Icerik', link: '/admin/news' },
    { id: 'activities', label: 'Faaliyetler', icon: 'fas fa-calendar-check', badge: activities.length, group: 'Icerik', link: '/admin/activities', active: true },
