...
export default function AdminNewsPage() {
  ...
  const fetchPageData = useCallback(async () => {
    try {
      const [s, n, msg, act, par, res] = await Promise.all([
      ...
      if (n.data) setNewsCount(n.data.length);
  ...
  // ✨ FORMU SIFIRLAMA FONKSİYONU (Zombi Veri Düşmanı) ✨
  const resetForm = () => {
    setIsEditing(false);
    setNewsForm({ id: null, title: '', title_en: '', summary: '', summary_en: '', description: '', description_en: '', image_url: '', date: '', gallery: [] });
    setFormKey(prev => prev + 1); // Formu tamamen öldürüp React'e baştan çizdirir
  };

 async function saveItem(e) {
    e.preventDefault();
    
    // dataToSave içine id'yi KESİNLİKLE koymuyoruz!
    ...
    let result;
    // Eğer id doluysa (yani düzenleme modundaysak) güncelle
    if (newsForm.id) {
      result = await supabase.from('news').update(dataToSave).eq('id', newsForm.id);
    } 
    // Eğer id null ise (yani YENİ ekliyorsak) id'siz olarak insert et
    else {
  ...
  function startEdit(item) {
    ...
    setFormKey(prev => prev + 1); // Temiz yükleme için form key'i değiştir
            ...
            <div className="adm-topbar-title">
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{
                  ...
                  <i className="fas fa-newspaper" style={{ color: '#fff', fontSize: '0.85rem' }} />
                </div>
                ...
                <form key={formKey} onSubmit={saveItem} style={{display:'grid', gap:'14px'}}>
                  ...
                  <div className="adm-form-grid2">
                      <div className="adm-form-item">
                        ...
                        <textarea className="adm-textarea-full" placeholder="2-3 cumle ozet..." value={newsForm.summary} onChange={e => setNewsForm(prev => ({...prev, summary: e.target.value}))} rows={3} />
                      </div>
                      <div className="adm-form-item">
                        <label>Kisa Ozet (EN)</label>
                        <textarea className="adm-textarea-full" placeholder="Short summary..." value={newsForm.summary_en} onChange={e => setNewsForm(prev => ({...prev, summary_en: e.target.value}))} rows={3} />
                      </div>
                  </div>
