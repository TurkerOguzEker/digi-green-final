...
export default function NewsDetailClient({ newsItem }) {
  ...
  // Eğer veritabanında 'gallery' diye bir sütun varsa ve JSON dizisi ise çözümlüyoruz.
  let galleryImages = [];
  if (newsItem.gallery) {
    try {
      galleryImages = Array.isArray(newsItem.gallery) ? newsItem.gallery : JSON.parse(newsItem.gallery);
    } catch (e) {
      galleryImages = [];
    }
  }
  ...
  const allImages = [
    newsItem.image_url || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?auto=format&fit=crop&q=80&w=1200',
    ...galleryImages
  ].filter(Boolean); // Boş olanları (null, undefined) at
    ...
    <div className="news-detail-page">
      ...
      {isLightboxOpen && (
        <div className="lightbox-overlay" onClick={closeLightbox}>
          ...
          {/* Altta Hangi Resimde Olduğunu Gösterme (Örn: 1 / 3) */}
          {allImages.length > 1 && (
      ...
      <style jsx>{`
        ...
        .hero-content {
          position: relative;
          z-index: 2;
          pointer-events: none; /* Yazının slider tıklamalarını engellememesi için */
        }
        .news-title {
      ...
      <style jsx global>{`
        ...
        /* QUILL EDITOR İÇERİĞİ İÇİN DÜZELTMELER */
        .quill-content {
          width: 100%;
          max-width: 100%;
          color: #444;
